...
import { marked } from 'marked' // Importando apenas para garantir que o pacote esteja instalado
...
function Welcome() {
  ...
  const [deepsekModalOpen, setDeepsekModalOpen] = useState(true)
  ...
  // Efeito para rolar para o final da lista de notas quando uma nova nota é adicionada
  useEffect(() => {
  ...
  // Efeito para carregar notas do localStorage ao iniciar
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center py-5 px-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h1 className="text-3xl font-extrabold text-blue-800 tracking-tight">TimeTracker</h1>
      </div>

      {/* Slogan */}
      <p className="text-gray-600 mb-6 max-w-md text-center">
        Bem-vindo ao <span className="font-semibold text-blue-700">TimeTracker</span>! Registre suas notas diárias, acompanhe seu desempenho e veja sua evolução ao longo do tempo.
      </p>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-2xl">
        <div className="max-h-[55vh] overflow-y-auto mb-4 space-y-3 notes-container">
          {notes.map((note, idx) => (
            <div key={idx} className="bg-blue-50 rounded-md px-3 py-2 shadow-sm">
              <div className="text-xs text-blue-400">{note.dateTime.toDateString()}</div>
              <div className="text-gray-800">{note.content}</div>
            </div>
          ))}
          {notes.length === 0 && (
            <div className="text-gray-400 text-center">Nenhuma nota ainda.</div>
          )}
        </div>
        <form
          ...
          <div className='flex items-center gap-2'>
            <textarea
              ...
              className="flex-1 px-3 py-2 rounded-md border border-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
              rows={1}
              onKeyDown={e => {
      ...
      <footer className="mt-8 text-xs text-gray-400">
        &copy; {new Date().getFullYear()} TimeTracker. Feito com ❤️ por você.
      </footer>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl relative">
        ...
        <h2 className="text-lg font-bold mb-4 text-blue-700">Configurações</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Chave de API do Deepsek</label>
            <input
              type="text"
              value={deepsekApiKey}
...
function GlobalSettingsButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      className="fixed top-4 right-4 z-50 bg-white border border-blue-200 rounded-full p-2 shadow hover:bg-blue-50 transition"
      onClick={onClick}
...
function DeepsekResponseModal({ open, onClose, response }: DeepsekResponseModalProps) {
  ...
  useEffect(() => {
    if (deepsekResponse) {
      ...
      if (!container) return
      container.innerHTML = marked.parse(deepsekResponse, {async: false}) as string;
    }
  }, [deepsekResponse])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl relative">
        ...
        <h2 className="text-lg font-bold mb-4 text-blue-700">Resposta do Deepsek</h2>
        <p className="text-sm text-gray-600 mb-4">
          A resposta do Deepsek será exibida abaixo. Se não houver resposta, uma mensagem padrão será mostrada.
        </p>
        <div className="whitespace-pre-line text-gray-800 max-h-[50vh] overflow-y-auto min-h-[3rem] flex items-center justify-center">
          {loading ? (
            ...
            <div className='md-content grid grid-cols-1'>
              {deepsekResponse || <span className="text-gray-400">Nenhuma resposta recebida.</span>}
            </div>
          )}
        </div>
      </div>
